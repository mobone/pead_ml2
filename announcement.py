...
class announcement():
    def __init__(eps_df, revenue_df):
        ...
        eps_history_df = pd.read_sql('select * from estimize_eps where Symbol == "%s"' % symbol, conn)
...
def get_estimize_data(self):
    ...
    url = 'https://www.estimize.com/calendar?tab=equity&date=' + datetime.now().strftime('%Y-%m-%d')
    self.driver.get(url)
